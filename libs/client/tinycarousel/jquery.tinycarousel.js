(function(factory) {
  ...
  (function($) {
    ...
    function Plugin($container, options) {
      ...
      , isHorizontal = this.options.axis === 'x', sizeLabel = isHorizontal ? "Width" : "Height", posiLabel = isHorizontal ? "left" : "top", intervalTimer = null;
      ...
      this.update = function() {
        ...
        if (self.options.children) {
          $slides = $overview.find(self.options.children);
        } else {
        ...
        if (self.options.infinite) {
          ...
          $overview.append($slides.slice(0, slidesVisible).clone(true).addClass("mirrored"));
        }
      ...
      function _setEvents() {
        if (self.options.buttons) {
          ...
          $next.click(function() {
            ...
            //以下为临时解决家居部分未解锁时，禁止滚动的问题
            if ($(this).hasClass('lock_img')) {
              return;
            }
            slideIndex += self.options.step;
            self.move(slideIndex);
      ...
      this.move = function(index) {
        ...
        if (slideIndex < 0) {
          self.slideCurrent = slideIndex = self.slidesTotal - self.options.step;
          $overview.css(posiLabel, -(self.slidesTotal) * slideSize);
        }

        if (slideIndex > self.slidesTotal) {
          self.slideCurrent = slideIndex = self.options.step;
      ...
      function _setButtons() {
        if (self.options.buttons && !self.options.infinite) {
          ...
          $next.toggleClass("disable", self.slideCurrent >= self.slidesTotal - slidesVisible - self.options.step);
        }
